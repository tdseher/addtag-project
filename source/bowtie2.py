...
def align(name, sequences, index, threads=(os.cpu_count() or 1), tempdir=os.getcwd(), options=None, sep=":"):
    """Aligns sequences using Bowtie 2
    
    Description of arguments:
      'name'    - the basename to give the generated files
      'tempdir' - generated files will be placed here
    ...
    """
    # bowtie2 -p 16 --very-fast-local -S ion.sam -x Corynebacterium-homunculus-11.24.2015 -U /home/thaddeus/homunculus/raw-reads/output.fastq 2> bowtie2.err
    # bowtie2 -p 16 --very-sensitive-local -S lib2-filter-align.sam -x /home/thaddeus/homunculus/thad-protocol-attempt1/test-lib2-reads/Corynebacterium-homunculus-11.24.2015 -1 /home/thaddeus/homunculus/thad-protocol-attempt2/lib2-filter/lib2-split-trimmed-nzl-prinseq-maxee-organize-concatenated/homunculus-lib2-r1-filtered-paired.fastq -2 /home/thaddeus/homunculus/thad-protocol-attempt2/lib2-filter/lib2-split-trimmed-nzl-prinseq-maxee-organize-concatenated/homunculus-lib2-r2-filtered-paired.fastq 2> bowtie2.err
    # bowtie2 -p ${THREADS} --rf --very-fast --end-to-end --no-overlap --minins 0 --maxins 8000 -S ${SUBDIR}/align-all-k${VELVET_KMER}.sam -x ${SUBDIR}/index-k${VELVET_KMER} -1 ${LONG_READ1_FASTQ} -2 ${LONG_READ2_FASTQ} 2> ${SUBDIR}/align-all-k${VELVET_KMER}.err
    
    sam_file = name + '.sam'
    query_file = name + '.fasta'
    error_file = name + '.err'
    
    # Create the query, converting list of sequences into a FASTA file
    with open(query_file, 'w') as flo:
        for line in sequences:
            contig, start, end, seq = line
            print(">" + sep.join(map(str, [contig, start, end])), file=flo)
            print(seq, file=flo)
            #print('+')
            #print('9'*len(seq))
    print('Query FASTA generated: {!r}'.format(query_file), file=sys.stderr)
...
def index_reference(fasta, tempdir=os.getcwd(), threads=(os.cpu_count() or 1), options=None, folder=None):
    ...
    print('FASTA file indexed: {!r}'.format(fasta), file=sys.stderr)
    print('Bowtie 2 index: {!r}'.format(index_file), file=sys.stderr)
